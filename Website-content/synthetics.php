...
?><? } ?>            
		...
		<hr />
         <pre>
         <?
         print_r($funds->visuals->swingfishSynth);
         ?></pre>
         ...
         <article class="post-item">
            <div class="post-body">
              <div class="post-meta">
                ...
                <div class="column"><span>updated: <?=$funds->visuals->swingfishSynth->updatedAgo?></span></div>
              </div><!-- .post-meta -->
...
new Chart(document.getElementById("myChart"), {
    type: 'bar',
    data: {
      labels: <?=$funds->visuals->swingfishSynth->chartcategories?>,
      datasets: [{
        ...
        },
{
    ...
    options: {
      title: {
        display: true,
        text: 'Last <?=$funds->visuals->swingfishSynth->lookback?> Trading days (Daytrades only)'
      },
      legend: { display: true }
    }
});</script><h3><?=round($funds->visuals->swingfishSynth->gain,2)?>% Gain</h3>
<?
